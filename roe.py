...
def harten_regularization(Lam, eps=1e-1):
    ...
    A_abs += (0.5 * ((1 + np.sign(eps - A_abs))
              * ((Lam**2 + eps**2) / (2*eps) - A_abs)))
    return A_abs


def comp_Q(u1, u2, c1sq, c2sq, c22sq, r, g, A, scheme='roe',
           eig_type='numerical', hyp_corr=True):
    ...
    c2sq: ndarray
        celerity of the lower layer (cw^2 = g * hw)
    r: flot or ndarray
    ...
    if eig_type == 'numerical':
        ...
        import analytic_eig
        Lam, K, F = analytic_eig.analytic_eig(u1, u2, c1sq, c2sq, c22sq, r, g,
                                              hyp_corr=hyp_corr)
    ...
    # Id = np.ones((Lam.shape[0], 4))
    # Pp = recompose_matrix(K, 0.5 * (Id + np.sign(Lam)))
    # Pm = recompose_matrix(K, 0.5 * (Id - np.sign(Lam)))
